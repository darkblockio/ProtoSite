...
//-H 'Connection: keep-alive' -H 'DNT: 1' -H 'Origin: https://arweave.net' --data-binary '{"query":"query {  transactions(    tags: { name: \"Uploading-App\", values: [\"Darkblock\"] }  ) {    edges {      node {        id tags { name value }      }    }  }}"}' --compressed
        ...
        async function getTheKey() {
          ...
          document.write('<div class="upload-container">');
            document.write(
            '<h1 style="margin-top:80px;" class="upload-header">Create a Darkblock enabled <span style="color:#00febb">GREEN</span> NFT</h1>'
          );
          ...
          document.write(
            "<h1 class=input-title >Carbon Offset</h1><select name=offset id=offset><br><br>"
          );
          document.write('<option value="0.01">0.01 AR</option>');
          document.write('<option value="0.1">0.1 AR</option>');
          document.write('<option value="0.5">0.5 AR</option>');
          document.write('<option value="1">1 AR</option>');
          document.write('<option value="2">2 AR</option>');
          document.write('<option value="10">10 AR</option>');
          document.write("</select>");

          document.write("</div>");
          document.write("</div>");
          document.write("<div style=margin-left:65px;>");
          document.write(
...
//          document.write('<div>');
//          document.write(
//            "<input type=checkbox id=royalty>I would like my NFT to offset its Carbon Footprint (Contribute 0.1 AR)"
//          );
//          document.write('</div>');
          document.write("</div>");
          document.write(
...
then close all select boxes:*/
    ...
    async function uploadMeta() {
        ...
        async function upload() {
          ...
          transaction.addTag("Artid", artid);
          transaction.addTag("Uploading-App", "Darkblock");
          transaction.addTag("Creator", artist);
          ...
          transaction.addTag(
            "Init-State",
            '{"name":"' + name + '","ticker":"DARK-' + Math.floor(Math.random() * 10000) + '","description":"' +
              description+ '", "balances":{"' + address + '":' + printcount + '},"vault":{},"votes":[],"roles":{},"settings":[["quorum",0.5],["support",0.5],["voteLength",2160],["lockMinLength",21600],["lockMaxLength",262800]]}'
          );
	      ...
	      $("body").html( 
	      '<nav>        <div>            <a href="index.html"><img width="100%" src="./images/logo.svg" alt=""></a>        </div>        <div class="nav-push">            <a class="nav-link" href="index.html">Home</a>            <a class="nav-link" href="/">Info</a><a class="nav-link" href="/">Account</a><a class="nav-link" href="/">Log Out</a>       </div></nav><main><section class="hero"><div class="container"><h1 class="hero-text">Darkblock <br><span class="artist-portal">Upload Complete!</span></h1><div>' + 
	      '<a class="button button-dark" href="green.html">Create another NFT</a>' + 
	      '<a class="button button-dark" href="https://space.verto.exchange/asset/' + nftTx + '">Sell my NFT on Space</a>' +
	      '<a class="button button-light" onclick=gallery()>View my NFTs</a></div></div>       </section>    </main>'
	      );
	  }
        ...
        async function sendOffset() {
          ...
          console.log("sending carbon offset ");
          let transaction = await arweave.createTransaction(
            {
    			target: '4niVAb_xIxfwmOcikXSqcOehl4n-OtahcZWNww006QQ',
    			quantity: arweave.ar.arToWinston(offset)
            },
            wallet
          );

          console.log("carbon offset transaction");

          await arweave.transactions.sign(transaction, wallet);
          console.log(transaction);
          var resp = await arweave.transactions.post(transaction);

          console.log("carbon offset complete with status " + resp.status);
        }
  ...
  files = document.getElementById("artfile").files;
  previewfiles = document.getElementById("previewfile").files;    
